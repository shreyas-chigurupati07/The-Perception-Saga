...
#        self.frontseg_path = os.path.join(self.absolute_path,'..','frontseg_masks/')
    ...
    def setRenderSettings(self):
        ...
        curr_filepath = curr_filepath.replace("main.blend","")
        output_folders = ["frames/","frontseg_masks/","segment_mask/"]
        idx = 0
    ...
    def render_imgs(self):
        ...
        for depth in range(10*self.limits["axis_depth"][0],10*self.limits["axis_depth"][1],int(10*self.limits["depth_step"])):
            for rotx in range(self.limits["axisrotd_x"][0],self.limits["axisrotd_x"][1],self.limits["rotd_step"]):
                for roty in range(self.limits["axisrotd_y"][0],self.limits["axisrotd_y"][1],self.limits["rotd_step"]):
                    for rotz in range(self.limits["axisrotd_z"][0],self.limits["axisrotd_z"][1],self.limits["rotd_step"]):
